...
# Regex pattern that captures common assistant-final markers emitted by reasoning models
ASSISTANT_FINAL_PATTERN = re.compile(
    r'(?i)(?:\\)?(?:<\|?)?\bassistant[\s_-]*final\b(?:\|?>)?'
)


def _focus_on_assistant_final(response_text: str) -> Tuple[str, bool]:
    """
    Trim reasoning-model outputs to just the assistantfinal section.

    Returns the (possibly) trimmed response text and a boolean indicating
    whether a trim occurred.
    """
    if not response_text:
        return response_text, False

    matches = list(ASSISTANT_FINAL_PATTERN.finditer(response_text))
    if not matches:
        return response_text, False

    trimmed_text = response_text[matches[-1].end():].strip()
    if not trimmed_text:
        # Nothing usable after the marker – fall back to original text
        return response_text, False
...
def generate_multi_agent_code(agents: List[Dict[str, Any]], edges: List[Tuple[str, str]]) -> str:
    ...
    code_lines = [
        ...
        "            combined_content = f'Original task: {original_task_info.content}; Current Sub-task: {agent_input}.'", #TODO: slightly different from minimal
        "            task_info = Info('task', 'User', combined_content, None, None, None, -1, None)",
        "",
...
def extract_harmony_code_from_response(response_text: str, validate_python_code, logger) -> Tuple[str, str, str]:
    ...
    try:
        ...
        parse_target, trimmed = _focus_on_assistant_final(response_text)
        if trimmed:
            main_rank_print("Detected reasoning-style output. Using assistantfinal section for parsing.")
        else:
            parse_target = response_text
        
        # Try to extract agents
        agents = extract_all_agents(parse_target)
        
        if agents:
            # Parse the agent response and generate code
            code = parse_harmony_agent_response(parse_target)
            ...
            thought = extract_xml(parse_target, "thinking")
            ...
            answer = extract_xml(parse_target, "answer")
            if answer:
                ...
                main_rank_print(f"No agents or answer found in harmony response - parsing failed. response_text: {parse_target}")
                return "direct_answer", "direct_answer", parse_target
