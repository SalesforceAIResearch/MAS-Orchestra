...
def save_building_blocks_evaluation_results(questions, batch, batch_building_block_outputs, config=None):
    ...
    # Use val_files directly in filename if config is provided
    val_files_suffix = ""
    if config is not None and hasattr(config, 'data') and hasattr(config.data, 'val_files'):
        val_files = config.data.val_files
        # Convert to string representation for filename
        if isinstance(val_files, list):
            val_files_str = "_".join(val_files)
        else:
            val_files_str = str(val_files)
        
        # Sanitize for filename (replace special characters)
        val_files_suffix = "_" + val_files_str.replace('/', '_').replace('.', '_').replace(' ', '_')
        pp.status("Dataset", f"Including val_files in filename: {val_files_suffix}", "info")
    ...
    if config is not None and hasattr(config, 'azr') and hasattr(config.azr, 'mas_r1') and hasattr(config.azr.mas_r1, 'agent') and hasattr(config.azr.mas_r1.agent, 'model_name'):
        agent_name = config.azr.mas_r1.agent.model_name
        # Sanitize for filename (replace special characters)
        agent_model_name = "_" + agent_name.replace('/', '_').replace('.', '_').replace(' ', '_')
        pp.status("Agent", f"Including agent model name in filename: {agent_model_name}", "info")
    ...
    if config is not None and hasattr(config, 'azr') and hasattr(config.azr, 'mas_r1') and hasattr(config.azr.mas_r1, 'agent') and hasattr(config.azr.mas_r1.agent, 'reasoning_effort'):
        ...
        # Sanitize for filename (replace special characters)
        reasoning_effort_suffix = "_" + str(reasoning_effort).replace('/', '_').replace('.', '_').replace(' ', '_')
        pp.status("Reasoning Effort", f"Including reasoning_effort in filename: {reasoning_effort_suffix}", "info")
    ...
    for block_name in building_blocks:
        ...
        # Save separate CSV file for this building block (include val_files, agent model name, and reasoning_effort)
        output_file = f"mas_r1_{mapped_name.lower().replace('-', '_')}{val_files_suffix}{agent_model_name}{reasoning_effort_suffix}_evaluation.csv"
