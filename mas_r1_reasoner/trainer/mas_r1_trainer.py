...
from mas_r1_reasoner.trainer.utils.log import (
    ...
    log_per_file_generations_to_wandb,
    collect_samples_by_file,
    accumulate_per_file_metrics,
    save_generated_code,
...
class MASR1Trainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        # Lists to collect samples grouped by source file
        samples_by_file = {}  # {source_file: {'inputs': [], 'outputs': [], 'scores': [], 'batches': [], 'reward_infos': []}}
        ...
        for batch_idx, batch_dict in enumerate(self.val_dataloader):
            ...
            # Collect samples by source file for per-file table logging
            samples_by_file = collect_samples_by_file(
            ...
            # Calculate scores for wandb logging and per-file metrics
            scores = reward_tensor.sum(-1).cpu().tolist()
            sample_scores.extend(scores)
            
            # Track source_file for per-file validation metrics
            if not hasattr(self, '_validation_per_file_metrics'):
                self._validation_per_file_metrics = {}
            
            # Accumulate per-file metrics
            self._validation_per_file_metrics = accumulate_per_file_metrics(
                test_batch=test_batch,
                reward_extra_info=reward_extra_info,
                scores=scores,
                per_file_metrics=self._validation_per_file_metrics
            )
        ...
        # Log per-file validation metrics if multiple validation files were provided
        if hasattr(self, '_validation_per_file_metrics') and len(self._validation_per_file_metrics) > 1:
            main_rank_print(f"\n{'='*60}")
            main_rank_print("PER-FILE VALIDATION METRICS")
            main_rank_print(f"{'='*60}")
            
            per_file_metrics = {}
            for source_file, metrics in self._validation_per_file_metrics.items():
                # Extract just the filename for cleaner logging
                import os
                file_name = os.path.basename(source_file)
                
                if metrics['overall_scores']:
                    avg_score = sum(metrics['overall_scores']) / len(metrics['overall_scores'])
                    avg_exec = sum(metrics['code_execution_success']) / len(metrics['code_execution_success'])
                    avg_answer = sum(metrics['final_answer_correctness']) / len(metrics['final_answer_correctness'])
                    
                    per_file_metrics[f'val/per_file/{file_name}/overall_score'] = avg_score
                    per_file_metrics[f'val/per_file/{file_name}/code_execution_success'] = avg_exec
                    per_file_metrics[f'val/per_file/{file_name}/final_answer_correctness'] = avg_answer
                    per_file_metrics[f'val/per_file/{file_name}/num_samples'] = len(metrics['overall_scores'])
                    
                    main_rank_print(f"\n{file_name}:")
                    main_rank_print(f"  - Samples: {len(metrics['overall_scores'])}")
                    main_rank_print(f"  - Overall Score: {avg_score:.4f}")
                    main_rank_print(f"  - Code Execution Success: {avg_exec:.4f}")
                    main_rank_print(f"  - Final Answer Correctness: {avg_answer:.4f}")
            
            # Log all per-file metrics to wandb
            if per_file_metrics:
                self.logger.log(data=per_file_metrics, step=self.global_steps)
            
            # Reset per-file metrics for next validation
            self._validation_per_file_metrics = {}
            main_rank_print(f"{'='*60}\n")
        ...
        # Log per-file validation tables if multiple validation files were provided
        if len(samples_by_file) > 1:
            main_rank_print(f"\n{'='*60}")
            main_rank_print("CREATING PER-FILE VALIDATION TABLES")
            main_rank_print(f"{'='*60}")
            
            if not hasattr(self, '_validation_per_file_tables'):
                self._validation_per_file_tables = {}
            
            for source_file, file_samples in samples_by_file.items():
                import os
                file_name = os.path.basename(source_file).replace('.parquet', '')
                
                main_rank_print(f"Creating table for {file_name}: {len(file_samples['inputs'])} samples")
                
                # Combine all single-sample batches into one batch for this file
                if file_samples['batches']:
                    # Combine batches using DataProto.concat
                    combined_batch = DataProto.concat(file_samples['batches'])
                    
                    # Merge reward_infos
                    combined_reward_info = {}
                    for key in file_samples['reward_infos'][0].keys():
                        combined_reward_info[key] = []
                        for reward_info in file_samples['reward_infos']:
                            if key in reward_info and reward_info[key]:
                                combined_reward_info[key].extend(reward_info[key])
                    
                    # Get or initialize table for this file
                    table_key = f'val_table_{file_name}'
                    file_table = self._validation_per_file_tables.get(table_key, None)
                    
                    # Log this file's samples using existing function
                    self._validation_per_file_tables[table_key] = log_per_file_generations_to_wandb(
                        inputs=file_samples['inputs'],
                        outputs=file_samples['outputs'],
                        scores=file_samples['scores'],
                        config=self.config,
                        global_steps=self.global_steps,
                        table=file_table,
                        test_batch=combined_batch,
                        reward_extra_info=combined_reward_info,
                        file_name=file_name
                    )
            
            main_rank_print(f"✅ Created {len(samples_by_file)} per-file validation tables")
            main_rank_print(f"{'='*60}\n")
